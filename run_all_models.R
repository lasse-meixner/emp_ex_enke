...
for (o in outcomes) {
  ...
  res_b  <- extract_results_BDML(fit_b,   method_name = "DML_LKJhalf")
  res_b$outcome <- o

  # 3.2 Inverse‐Wishart
  fit_b2 <- fit_model_dml_b2(o)
  res_b2 <- extract_results_BDML(fit_b2,  method_name = "DML_IW")
  res_b2$outcome <- o

  # 3.3 Simple LKJ (“R2D2”)
  fit_r2 <- fit_model_dml_r2d2(o)
  res_r2 <- extract_results_BDML(fit_r2,  method_name = "R2D2_LKJ")
  res_r2$outcome <- o
...
write_csv(final_results, "results.csv")